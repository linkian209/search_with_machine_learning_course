    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        for hit in response["hits"]["hits"]:
            ...
            for feature in hit["fields"]["_ltrlog"][0]["log_entry"]:
                if feature["name"] == "salesRankShortTerm":
                    if "value" in feature:
                        feature_results["salesRankShortTerm"].append(feature["value"])
                    else:
                        feature_results["salesRankShortTerm"].append(None)

                if feature["name"] == "salesRankMediumTerm":
                    if "value" in feature:
                        feature_results["salesRankMediumTerm"].append(feature["value"])
                    else:
                        feature_results["salesRankMediumTerm"].append(None)

                if feature["name"] == "salesRankLongTerm":
                    if "value" in feature:
                        feature_results["salesRankLongTerm"].append(feature["value"])
                    else:
                        feature_results["salesRankLongTerm"].append(None)

                if feature["name"] == "salePrice":
                    if "value" in feature:
                        feature_results["salePrice"].append(feature["value"])
                    else:
                        feature_results["salePrice"].append(None)

                if feature["name"] == "name_match":
                    if "value" in feature:
                        feature_results["name_match"].append(feature["value"])
                    else:
                        feature_results["name_match"].append(None)
